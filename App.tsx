...
export default function PDFToImageConverter() {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Scale 2.0 provides good quality for export
      ...
      alert('讀取 PDF 失敗，請確認檔案是否損毀或有密碼保護。');
    } finally {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === images.length) {
      ...
      const newSet = new Set(images.map(img => img.id));
      setSelectedIds(newSet); // Select all
    }
  };
    ...
    try {
      ...
      images.forEach(img => {
        if (selectedIds.has(img.id)) {
          // Remove header "data:image/jpeg;base64," to get raw base64
  ...
  const downloadSelectedLongImage = async () => {
    ...
    try {
      // 1. Get selected images in correct order (1, 2, 3...)
      ...
      // 2. Calculate dimensions for the long canvas
      // Width: Max width of selected images
      // Height: Sum of heights
      ...
      // 4. Draw images one by one
      let currentY = 0;
      for (const imgObj of selectedImages) {
        const image = new Image();
        image.src = imgObj.data;
        
        // Wait for image to load before drawing (even base64 needs this sometimes in strict modes)
          ...
          <div className="flex items-center gap-3">
            {pdfFile && !isProcessing && (
              <>
                <button 
                  onClick={resetAll}
                  className="flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-800 hover:bg-red-500/20 hover:text-red-400 text-slate-400 rounded-lg transition-colors border border-transparent hover:border-red-500/50"
                >
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {progress.total} 頁 • 已選取 {selectedIds.size} 頁
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {selectedIds.size === images.length && images.length > 0 ? (
                      <><CheckSquare className="w-4 h-4 text-blue-400" /> 取消全選</>
                    ) : (
                  ...
                  >
                    {isZipping ? (
                      ...
                      <><Package className="w-4 h-4" /> 下載 ZIP</>
                    )}
                  </button>

                  <button
                    onClick={downloadSelectedLongImage}
                    disabled={selectedIds.size === 0 || isZipping || isStitching}
                    className={`
                      flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all
                      ${selectedIds.size > 0 && !isZipping && !isStitching
                        ? 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-500/25 active:scale-95' 
                        : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'}
                    `}
                  >
                    {isStitching ? (
                      <><Loader2 className="w-4 h-4 animate-spin" /> 拼接中...</>
                    ) : (
                      <><Layers className="w-4 h-4" /> 下載長圖</>
                    )}
                  </button>
                </div>
              )}
              ...
              {images.map((img) => {
                ...
                return (
                  <div 
                    key={img.id} 
                    className={`
                      group relative bg-slate-800 rounded-2xl overflow-hidden border shadow-xl transition-all duration-200
                      ${isSelected ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-700 hover:border-slate-500'}
                    `}
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 z-10 cursor-pointer">
                      <div className={`
                        w-6 h-6 rounded-md flex items-center justify-center transition-all duration-200 shadow-md border
                        ${isSelected 
                          ? 'bg-blue-500 border-blue-500 text-white' 
                          : 'bg-slate-900/80 border-slate-500 text-transparent hover:border-white'}
                      `}>
                    ...
                    <div className="aspect-[3/4] bg-slate-900 w-full overflow-hidden flex items-center justify-center relative cursor-pointer">
                      ...
                      {/* Hover Overlay for individual download */}
                      <div 
                        className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                        onClick={(e) => e.stopPropagation()} // Prevent selection toggle when clicking download
                      >
                         <button
                          onClick={() => downloadImage(img.data, img.id)}
                          className="bg-white/90 text-slate-900 font-bold py-2 px-4 rounded-full flex items-center gap-2 hover:bg-white active:scale-95 transition-transform text-sm shadow-xl backdrop-blur-sm"
                        >
                    ...
                    <div className="p-3 bg-slate-800 border-t border-slate-700 flex justify-between items-center" onClick={(e) => e.stopPropagation()}>
                      <span className={`text-sm font-medium flex items-center gap-2 transition-colors ${isSelected ? 'text-blue-400' : 'text-slate-300'}`}>
